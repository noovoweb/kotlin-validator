...
object CustomValidators {
    suspend fun validateUsername(value: String?, @Suppress("UNUSED_PARAMETER") context: ValidationContext,): Boolean {
        if (value == null) return true
        return value.length >= MIN_USERNAME_LENGTH && value.all { it.isLetterOrDigit() || it == '_' }
    }

    suspend fun validateEvenNumber(value: Int?, @Suppress("UNUSED_PARAMETER") context: ValidationContext,): Boolean {
        if (value == null) return true
        return value % 2 == 0
    }

...
class MinSizeListValidatorTest {
    @Test
    fun `minsize validator accepts lists at or above minimum`() =
        runTest {
    ...
    @Test
    fun `minsize validator rejects lists below minimum`() =
        runTest {
            val validator = MinSizeListValidator()
    ...
    @Test
    fun `minsize validator allows null`() =
        runTest {
            MinSizeListValidator().validate(MinSizeList(items = null))
        }

    @Test
    fun `minsize validator provides error message`() =
        runTest {
            val exception =

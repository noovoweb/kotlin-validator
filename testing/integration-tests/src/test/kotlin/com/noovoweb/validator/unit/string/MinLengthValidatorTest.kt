...
class MinLengthValidatorTest {
    ...
    @Test
    fun `minlength validator accepts strings at or above minimum`() = runTest {
    ...
    @Test
    fun `minlength validator rejects strings below minimum`() = runTest {
        val validator = MinLengthValidator()
    ...
    @Test
    fun `minlength validator allows null when not required`() = runTest {
        val validator = MinLengthValidator()
        validator.validate(MinLength(name = null))
    }

    @Test
    fun `minlength validator provides error message`() = runTest {
        val validator = MinLengthValidator()

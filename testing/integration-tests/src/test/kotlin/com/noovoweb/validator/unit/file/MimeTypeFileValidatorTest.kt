...
class MimeTypeFileValidatorTest {
    ...
    fun `mime type validator rejects invalid mime types`(
        ...
        val pdfFile =
            File(tempDir, "document.pdf").apply {
                createNewFile()
    ...
    @Test
    fun `mime type validator allows null when not required`() =
        runTest {
            val validator = MimeTypeFileValidator()

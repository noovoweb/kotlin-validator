...
class TodayDateValidatorTest {
    @Test
    fun `today validator accepts today's date`() =
        runTest {
    ...
    @Test
    fun `today validator rejects past dates`() =
        runTest {
            val fixedClock = Clock.fixed(Instant.parse("2024-11-16T00:00:00Z"), ZoneId.of("UTC"))
            val context = ValidationContext(clock = fixedClock)
            val validator = TodayDateValidator()
    ...
    @Test
    fun `today validator rejects future dates`() =
        runTest {
            val fixedClock = Clock.fixed(Instant.parse("2024-11-16T00:00:00Z"), ZoneId.of("UTC"))
            val context = ValidationContext(clock = fixedClock)
            val validator = TodayDateValidator()
    ...
    @Test
    fun `today validator allows null when not required`() =
        runTest {
            val validator = TodayDateValidator()
    ...
    @Test
    fun `today validator provides error message`() =
        runTest {
            val fixedClock = Clock.fixed(Instant.parse("2024-11-16T00:00:00Z"), ZoneId.of("UTC"))
            val context = ValidationContext(clock = fixedClock)
            val validator = TodayDateValidator()

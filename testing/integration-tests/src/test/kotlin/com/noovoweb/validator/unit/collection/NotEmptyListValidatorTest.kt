...
class NotEmptyListValidatorTest {
    @Test
    fun `notempty validator accepts non-empty lists`() = runTest {
    ...
    @Test
    fun `notempty validator rejects empty lists`() = runTest {
        val validator = NotEmptyListValidator()
        assertThrows<ValidationException> { validator.validate(NotEmptyList(items = emptyList())) }
    }

    @Test
    fun `notempty validator allows null`() = runTest {
        NotEmptyListValidator().validate(NotEmptyList(items = null))
    }

    @Test
    fun `notempty validator provides error message`() = runTest {

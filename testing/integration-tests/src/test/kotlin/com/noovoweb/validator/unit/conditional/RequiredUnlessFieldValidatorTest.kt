...
class RequiredUnlessFieldValidatorTest {
    @Test
    fun `requiredunless validator accepts when exception not met and field present`() = runTest {
        val validator = RequiredUnlessFieldValidator()
        validator.validate(RequiredUnlessField(paymentMethod = "card", cardNumber = "1234"))
    }

    @Test
    fun `requiredunless validator accepts when exception met`() = runTest {
        val validator = RequiredUnlessFieldValidator()
        validator.validate(RequiredUnlessField(paymentMethod = "cash", cardNumber = null))
    }

    @Test
    fun `requiredunless validator rejects when exception not met but field absent`() = runTest {
        val validator = RequiredUnlessFieldValidator()
    ...
    @Test
    fun `requiredunless validator provides error message`() = runTest {

...
class PastDateValidatorTest {
    ...
    @Test
    fun `past validator accepts past dates`() = runTest {
    ...
    @Test
    fun `past validator rejects future dates`() = runTest {
        val fixedClock = Clock.fixed(Instant.parse("2024-11-16T00:00:00Z"), ZoneId.of("UTC"))
        val context = ValidationContext(clock = fixedClock)
        val validator = PastDateValidator()
    ...
    @Test
    fun `past validator rejects today`() = runTest {
        val fixedClock = Clock.fixed(Instant.parse("2024-11-16T00:00:00Z"), ZoneId.of("UTC"))
        val context = ValidationContext(clock = fixedClock)
        val validator = PastDateValidator()
    ...
    @Test
    fun `past validator allows null when not required`() = runTest {
        val validator = PastDateValidator()
        validator.validate(PastDate(date = null))
    }

    @Test
    fun `past validator provides error message`() = runTest {
        val fixedClock = Clock.fixed(Instant.parse("2024-11-16T00:00:00Z"), ZoneId.of("UTC"))
        val context = ValidationContext(clock = fixedClock)
        val validator = PastDateValidator()

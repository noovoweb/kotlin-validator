...
class EndsWithValidatorTest {
    ...
    @Test
    fun `endswith validator accepts strings with correct suffix`() = runTest {
    ...
    @Test
    fun `endswith validator rejects strings without correct suffix`() = runTest {
        val validator = EndsWithValidator()
    ...
    @Test
    fun `endswith validator allows null when not required`() = runTest {
        val validator = EndsWithValidator()
        validator.validate(EndsWith(name = null))
    }

    @Test
    fun `endswith validator provides error message`() = runTest {
        val validator = EndsWithValidator()

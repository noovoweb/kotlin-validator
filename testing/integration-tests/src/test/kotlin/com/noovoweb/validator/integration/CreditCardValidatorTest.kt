...
class CreditCardValidatorTest {
    @Test
    fun `@CreditCard should pass with valid Visa card`() = runTest {
    ...
    @Test
    fun `@CreditCard should pass with valid MasterCard`() = runTest {
        val validator = CreditCardTestDataValidator()
        validator.validate(CreditCardTestData(cardNumber = "5425233430109903"))
    }

    @Test
    fun `@CreditCard should pass with valid American Express`() = runTest {
        val validator = CreditCardTestDataValidator()
        validator.validate(CreditCardTestData(cardNumber = "378282246310005"))
    }

    @Test
    fun `@CreditCard should pass with valid Discover`() = runTest {
        val validator = CreditCardTestDataValidator()
        validator.validate(CreditCardTestData(cardNumber = "6011111111111117"))
    }

    @Test
    fun `@CreditCard should pass with null value`() = runTest {
        val validator = CreditCardTestDataValidator()
        validator.validate(CreditCardTestData(cardNumber = null))
    }

    @Test
    fun `@CreditCard should fail with invalid card number`() = runTest {
        val validator = CreditCardTestDataValidator()
        val exception =
    ...
    @Test
    fun `@CreditCard should fail with wrong length for card type`() = runTest {
        val validator = CreditCardTestDataValidator()
    ...
    @Test
    fun `@CreditCard should fail with non-digit characters`() = runTest {
        val validator = CreditCardTestDataValidator()
        val exception =
            assertThrows<ValidationException> {
    ...
    @Test
    fun `@CreditCard should fail with too short number`() = runTest {
        val validator = CreditCardTestDataValidator()
        val exception =
            assertThrows<ValidationException> {

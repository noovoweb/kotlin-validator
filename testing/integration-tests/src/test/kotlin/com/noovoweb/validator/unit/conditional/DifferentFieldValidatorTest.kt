...
class DifferentFieldValidatorTest {
    @Test
    fun `different validator accepts different values`() =
        runTest {
    ...
    @Test
    fun `different validator rejects same values`() =
        runTest {
            val validator = DifferentFieldValidator()
    ...
    @Test
    fun `different validator accepts when one is null`() =
        runTest {
            DifferentFieldValidator().validate(DifferentField(currentPassword = "old", newPassword = null))
        }

    @Test
    fun `different validator provides error message`() =
        runTest {
            val exception =
                assertThrows<ValidationException> {

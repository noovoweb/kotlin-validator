...
class FutureDateValidatorTest {
    @Test
    fun `future validator accepts future dates`() =
        runTest {
    ...
    @Test
    fun `future validator rejects past dates`() =
        runTest {
            val fixedClock = Clock.fixed(Instant.parse("2024-11-16T00:00:00Z"), ZoneId.of("UTC"))
            val context = ValidationContext(clock = fixedClock)
            val validator = FutureDateValidator()
    ...
    @Test
    fun `future validator rejects today`() =
        runTest {
            val fixedClock = Clock.fixed(Instant.parse("2024-11-16T00:00:00Z"), ZoneId.of("UTC"))
            val context = ValidationContext(clock = fixedClock)
            val validator = FutureDateValidator()
    ...
    @Test
    fun `future validator allows null when not required`() =
        runTest {
            val validator = FutureDateValidator()
    ...
    @Test
    fun `future validator provides error message`() =
        runTest {
            val fixedClock = Clock.fixed(Instant.parse("2024-11-16T00:00:00Z"), ZoneId.of("UTC"))
            val context = ValidationContext(clock = fixedClock)
            val validator = FutureDateValidator()

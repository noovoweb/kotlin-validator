...
class MacAddressDataValidatorTest {
    ...
    @Test
    fun `mac validator accepts valid MAC addresses`() = runTest {
    ...
    @Test
    fun `mac validator rejects invalid MAC addresses`() = runTest {
        val validator = MacAddressDataValidator()
    ...
    @Test
    fun `mac validator allows null when not required`() = runTest {
        val validator = MacAddressDataValidator()
        validator.validate(MacAddressData(address = null))
    }

    @Test
    fun `mac validator provides error message`() = runTest {
        val validator = MacAddressDataValidator()

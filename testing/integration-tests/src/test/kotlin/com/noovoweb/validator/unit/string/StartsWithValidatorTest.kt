...
class StartsWithValidatorTest {
    ...
    @Test
    fun `startswith validator accepts strings with correct prefix`() = runTest {
    ...
    @Test
    fun `startswith validator rejects strings without correct prefix`() = runTest {
        val validator = StartsWithValidator()
    ...
    @Test
    fun `startswith validator allows null when not required`() = runTest {
        val validator = StartsWithValidator()
        validator.validate(StartsWith(name = null))
    }

    @Test
    fun `startswith validator provides error message`() = runTest {
        val validator = StartsWithValidator()

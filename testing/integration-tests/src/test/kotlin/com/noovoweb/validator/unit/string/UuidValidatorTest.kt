...
class UuidValidatorTest {
    ...
    @Test
    fun `uuid validator accepts valid UUID formats`() = runTest {
    ...
    @Test
    fun `uuid validator accepts lowercase and uppercase UUIDs`() = runTest {
        val validator = UuidValidator()
    ...
    @Test
    fun `uuid validator rejects invalid UUID formats`() = runTest {
        val validator = UuidValidator()
        ...
        val exception5 = assertThrows<ValidationException> {
    ...
    @Test
    fun `uuid validator allows null when not required`() = runTest {
        val validator = UuidValidator()
        validator.validate(Uuid(name = null))
    }

    @Test
    fun `uuid validator provides error message`() = runTest {
        val validator = UuidValidator()

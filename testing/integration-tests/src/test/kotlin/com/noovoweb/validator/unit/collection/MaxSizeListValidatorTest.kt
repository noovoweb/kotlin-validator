...
class MaxSizeListValidatorTest {
    @Test
    fun `maxsize validator accepts lists at or below maximum`() =
        runTest {
    ...
    @Test
    fun `maxsize validator rejects lists above maximum`() =
        runTest {
            val validator = MaxSizeListValidator()
            assertThrows<ValidationException> {
                validator.validate(MaxSizeList(items = listOf("a", "b", "c", "d", "e", "f")))
            }
        }

    @Test
    fun `maxsize validator allows null`() =
        runTest {
            MaxSizeListValidator().validate(MaxSizeList(items = null))
        }

    @Test
    fun `maxsize validator provides error message`() =
        runTest {
            val exception =
                assertThrows<ValidationException> {

...
class RequiredWithFieldValidatorTest {
    @Test
    fun `requiredwith validator accepts when any dependency present and field present`() =
        runTest {
    ...
    @Test
    fun `requiredwith validator accepts when no dependencies present`() =
        runTest {
            val validator = RequiredWithFieldValidator()
    ...
    @Test
    fun `requiredwith validator rejects when dependency present but field absent`() =
        runTest {
            val validator = RequiredWithFieldValidator()
    ...
    @Test
    fun `requiredwith validator provides error message`() =
        runTest {
            val exception =
                assertThrows<ValidationException> {

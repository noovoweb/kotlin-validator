...
public sealed class ValidationResult<out T> {
    ...
    public inline fun getOrElse(defaultValue: (Map<String, List<ValidationError>>) -> @UnsafeVariance T): @UnsafeVariance T = when (this) {
        is Success -> value
    ...
    public inline fun <R> map(transform: (T) -> R): ValidationResult<R> = when (this) {
        is Success -> Success(transform(value))
        is Failure -> this
    }

    public inline fun <R> flatMap(transform: (T) -> ValidationResult<R>): ValidationResult<R> = when (this) {

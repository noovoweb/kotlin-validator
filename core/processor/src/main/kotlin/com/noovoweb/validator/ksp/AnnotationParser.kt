...
internal class AnnotationParser(private val logger: KSPLogger) {
    ...
    internal fun parse(classDeclaration: KSClassDeclaration): ValidatedClassInfo? {
        ...
        // Collect all property names for cross-field reference validation
        val allPropertyNames = classDeclaration.getAllProperties()
        ...
        val properties =
            classDeclaration.getAllProperties()
                .mapNotNull { property -> parseProperty(property, allPropertyNames, classDeclaration) }
                .toList()
    ...
    private fun parseProperty(
                ...
                // Parse validation validators
                    ...
                    parseValidationValidator(annotation)?.let { validator ->
                        ...
                        ?: run {
                            ...
                            // Silently skip non-validator annotations (e.g., @Serializable, @JsonProperty)
                        }
                }
            }
        ...
        // Validate cross-field references at compile time
        validators.forEach { validator ->
            val referencedFields = when (validator) {
                is ValidationValidatorInfo.SameValidator -> listOf(validator.field)
                is ValidationValidatorInfo.DifferentValidator -> listOf(validator.field)
                is ValidationValidatorInfo.RequiredIfValidator -> listOf(validator.field)
                is ValidationValidatorInfo.RequiredUnlessValidator -> listOf(validator.field)
                is ValidationValidatorInfo.RequiredWithValidator -> validator.fields
                is ValidationValidatorInfo.RequiredWithoutValidator -> validator.fields
                else -> emptyList()
            }
            referencedFields.forEach { fieldName ->
                if (fieldName !in allPropertyNames) {
                    logger.error(

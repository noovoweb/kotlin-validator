...
Exception handling is **your responsibility** — the library throws `ValidationException` on validation failure, and you handle it in your framework's error handling layer (e.g., `@ControllerAdvice` in Spring). This gives you full control over error response format and status codes.
...
The `kotlin-validator-ktor` module provides seamless integration with Ktor using its plugin system. It offers internationalization and native coroutine support.
...
Exception handling is **your responsibility** — the library throws `ValidationException` on validation failure, and you handle it in your framework's error handling layer (e.g., Ktor `StatusPages` plugin). This gives you full control over error response format and status codes.
...
**Response errors in French (format depends on your error handler):**
...
| **Error Handling** | User-defined (e.g., `@ControllerAdvice`) | User-defined (e.g., `@ControllerAdvice`) | User-defined (e.g., `StatusPages`) |
...
**All modules provide:**
- ✅ `ValidationException` with structured errors for easy error handling
...
| `@Luhn` | Luhn algorithm check | `@Luhn val identifier: String?` |
| `@CreditCard` | Valid credit card number | `@CreditCard val cardNumber: String?` |
...
// RUN JVM clean build and publish to local Maven repo
```bash

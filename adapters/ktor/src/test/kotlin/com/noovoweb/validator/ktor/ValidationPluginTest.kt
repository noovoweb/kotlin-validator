...
class ValidationPluginTest {
    ...
    @Test
    fun `plugin should be installed successfully`() =
        testApplication {
            application {
    ...
    @Test
    fun `plugin should handle ValidationException with 422 status`() =
        testApplication {
            application {
                install(ValidationPlugin)
    ...
    @Test
    fun `validationContext should extract locale from Accept-Language header`() =
        testApplication {
    ...
    @Test
    fun `validationContext should use default locale when header is missing`() =
        testApplication {
            var capturedLocale: Locale? = null
    ...
    @Test
    fun `ValidationErrorResponse should be properly serialized`() =
        testApplication {
            application {
                install(ValidationPlugin)
                install(ContentNegotiation) {
    ...
    @Test
    fun `validationContext should throw exception when plugin not installed`() =
        testApplication {
            application {
                // No plugin installed
            }

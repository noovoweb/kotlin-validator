 ...
 * in Spring MVC module, including fallback to core messages.
 */
class SpringMvcMessageTranslationTest {

    @Test
    fun `Spring MVC module falls back to core translations for built-in validators`() = runTest {
    ...
    @Test
    fun `Spring MessageSource can override core translations`() = runTest {
        val messageSource = StaticMessageSource()
        messageSource.addMessage("field.required", Locale.ENGLISH, "Custom: This field is mandatory")
        messageSource.addMessage("field.required", Locale.FRENCH, "Personnalisé: Ce champ est requis")
        ...
        assertEquals("Custom: This field is mandatory", englishMessage)
        assertEquals("Personnalisé: Ce champ est requis", frenchMessage)
    }

    @Test
    fun `custom Spring messages work with parameters`() = runTest {
        val messageSource = StaticMessageSource()
    ...
    @Test
    fun `Spring MVC can use application-specific message bundles`() = runTest {
    ...
    @Test
    fun `all core validator messages are accessible through Spring MVC`() = runTest {
        val messageSource = StaticMessageSource()
        val provider = SpringMessageProvider(messageSource)
        ...
        allKeys.forEach { key ->
            ...
            assertEquals(coreEnglish, springEnglish, "Spring MVC should fall back to core for $key (English)")
            assertEquals(coreFrench, springFrench, "Spring MVC should fall back to core for $key (French)")
        }
    }

    @Test
    fun `Spring MVC preserves parameter interpolation for translated messages`() = runTest {
        val messageSource = StaticMessageSource()
        val provider = SpringMessageProvider(messageSource)
    ...
    @Test
    fun `Spring MessageSource priority is correct - custom before default`() = runTest {
        val messageSource = StaticMessageSource()
    ...
    @Test
    fun `Spring MVC handles missing custom translations gracefully`() = runTest {
        val messageSource = StaticMessageSource()
    ...
    @Test
    fun `Spring MVC supports locale-specific message resolution`() = runTest {
        val messageSource = StaticMessageSource()
    ...
    @Test
    fun `all validator categories are translated through Spring MVC`() = runTest {
        val messageSource = StaticMessageSource()
        val provider = SpringMessageProvider(messageSource)

...
class ReactiveValidationExtensionsTest {
        ...
        override suspend fun validateResult(
            ...
            return try {
                ...
                val validationErrors =
                    ex.errors.mapValues { (_, messages) ->
    ...
    @Test
    fun `validateMono should complete successfully for valid data`() =
        runTest {
    ...
    @Test
    fun `validateMono should throw ValidationException for invalid data`() =
        runTest {
    ...
    @Test
    fun `validateMono can be chained with other Mono operations`() =
        runTest {
            val validator = TestRequestValidator(shouldFail = false)
            val request = TestRequest(email = "test@example.com", name = "Alice")
            val context = ValidationContext()
            ...
            val result =
                validator.validateMono(request, context)
                    .thenReturn("Success")
                    .block()
